...
rule(sq([A1,A2,A3],[A4,0,A6],[A7,A8,A9]),0) :- 
	sum([A1,A2,A3,A4,A6,A7,A8,A9],X), member(X,[0,1,2,4,5,6,7,8]).
rule(sq([A1,A2,A3],[A4,1,A6],[A7,A8,A9]),0) :-
	sum([A1,A2,A3,A4,A6,A7,A8,A9],X), member(X,[1,4,5,6,7,8,9]).
...
member(A,[A|Xs]).
member(A,[B|Xs]) :- member(A,Xs).

l(X) :- life(X,Y), wl(X), read('s'), l(Y).
